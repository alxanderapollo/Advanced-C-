...
string BigInt ::sumAndCarry(string& a,string& b){
    ...
    while(iterator < a_length){
        mySum = a[iterator]-'0'; //return asci integer representation of the char a[i] (first group of nums)
        mySum+= b[iterator]-'0';//return asci integer representation of the char b[i] (second group of nums)
        mySum+= carryMe;
        //store our resulting sum
        strResult.push_back((mySum%10) + '0');
        carryMe = mySum/10; // any carry is added in the next iteration
...
BigInt& BigInt::operator/(BigInt const &obj){
    ...
    a = convertToString(this->myStr,this->length);//convert a to a string
    b = convertToString(obj.myStr,obj.length);//convert b to a string 
    //2.if a is smaller than b return zero
    //doesnt return the remainder
    strResult = divideAndCarry(a,b);
    //3. convert back to a c-String for the object
    rsLength = strResult.length();
    char char_array[rsLength + 1];
    //copying the contents of the
...
string BigInt::divideAndCarry(string& a, string& b){
    // quick check
    stringstream sa(a); 
    stringstream sb(b); 
    long x = 0; 
    ...
    //if a == b return 1
    //if b == 0, return NAN 
    // if a < b, Return prompt "try a number for A: bigger than b"
    //--->this program doesnt do remainders
    //otherwise do divsion normally
    if(b == "0"){
        string myResult; 
        myResult = "Not A Number, No Division by zero";
        return myResult;
    }else if(a == "0") { // if a is zero any number that is going to be used to divide agianst this will be zero anyway, so return 0
        string myResult; 
        myResult = "0";
        return myResult;
    }
    else if(a == b){ //if a and be are the same there is no point to run the entire program =
        string myResult; 
        myResult = "1";
        return myResult;
    }else if(a.length() <b.length() || x < y){ //if b has a bigger length then most likely b is bigger number, which this program doesn't support
        string myResult; 
        myResult = "This program does not do remainders, please enter a bigger A value to be divded by a Smaller B Value";
        return myResult;
    }else{ //a > b  we can divde like normal
        string Quotient; 
        int i = 0, myA = a[i] - '0',divisor = b[i] - '0', primeFactorB = 0,
        a_prefix, holdMe, a_length, carry;
        a_length = a.size();
        //b is converted into an int
        stringstream tempHold(b);
        tempHold >> primeFactorB;
        //tempHold = b which was changed into an int
        //primeFactor = b
        //temp = b[0]
        //a_prefix = a[1]
        ...
        //b = 123
        do{
            //myresult will store each number
            int temp = myA / divisor; // divide
            carry = myA % divisor; //get remiander
            myA *= 10; 
            Quotient += ((temp + carry) + ' 0');//transform into a char digit  and save
        }  while (a_length > i);
        //4. return result
        return Quotient;
    }
} //end divide and carry
// print out result
void BigInt::printMe(){
    //print string
